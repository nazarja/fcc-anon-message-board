...
module.exports = function (app) {
  ...
  .get((req, res) => {
    ...
    MongoClient.connect(DATABASE, (err, db) => {
      db.collection(board).find({reported: false}).limit(10).sort({bumped_at: -1}).toArray( (err, result) => {
  ...
  .put((req, res) => {
    ...
    MongoClient.connect(DATABASE, (err, db) => {
      db.collection(board).updateOne({_id: _id}, {$set: {reported: true}}, (err, result) => {
        if (err) console.log(err);
  ...
  .delete((req, res) => {
    ...
    MongoClient.connect(DATABASE, (err, db) => {
      db.collection(board).findOneAndDelete({_id: _id, delete_password: password}, (err, result) => {
        if (err) console.log(err);
        console.log(result)
  ...
  .get((req, res) => {
    ...
    MongoClient.connect(DATABASE, (err, db) => {
      db.collection(board).findOne({_id: _id}, (err, doc) => {
        if (err) console.log(err);
  ...
  .post((req, res) => {
    ...
    MongoClient.connect(DATABASE, (err, db) => {
      db.collection(board).updateOne({_id: _id}, {$set: {bumped_on: obj.created_on}, $push: {replies: obj}}, (err, result) => {
        if (err) console.log(err);
    ...
    function update(replies) {
      ...
      for (let item of replies) {
          if (item._id && item._id == reply_id) {
            ...
            MongoClient.connect(DATABASE, (err, db) => {
              db.collection(board).updateOne({_id: _id}, {$set: {replies: replies}}, (err, result) => {
                if (err) console.log(err);
